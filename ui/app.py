...
# ── Data-аксессоры: ВСЕГДА API-first, mock — только fallback (работает с бэкендом A). ──
def get_models():
    return _api_get("/api/v1/models", mock=MOCK_MODELS) or MOCK_MODELS


def get_versions(name):
    fb = MOCK_MODEL_VERSIONS.get(name, [])
    return _api_get(f"/api/v1/models/{name}/versions", mock=fb) or fb


def get_dataset(key):
    return _api_get(f"/api/v1/datasets/{key}", mock=MOCK_DATASETS.get(key)) or MOCK_DATASETS.get(key)


def get_users():
    return _api_get("/api/v1/admin/users", mock=MOCK_USERS) or MOCK_USERS


def get_resources():
    return _api_get("/api/v1/resources", mock=MOCK_RESOURCES) or MOCK_RESOURCES


def get_controls():
    fb = CTRL.CONTROLS if CTRL else []
    return _api_get("/api/v1/controls", mock=fb) or fb
...
def page_dashboard():
    ...
    n_models = len(models)
    n_find = len([f for f in findings if f["status"] == "open"])
    n_events = len(events)
    n_hitl = len([m for m in models if m["status"] == "pending_hitl"])
...
def page_findings():
    ...
    data = _api_get("/api/v1/findings", mock=MOCK_FINDINGS) or MOCK_FINDINGS
    if f_gate:
...
def page_cicd():
    if _role() != "MLSecOps":
        ...
        return
    runs = _api_get("/api/v1/cicd/runs", mock=MOCK_CICD_RUNS) or MOCK_CICD_RUNS
    c1, c2 = st.columns([3, 1])
...
def page_deploy():
    ...
    models = [m for m in get_models() if m["status"] in ("candidate", "pending_hitl")]
    if not models:
