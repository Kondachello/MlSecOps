...
def _check_presidio_pii(df) -> dict:
    """Presidio analyzer — доп. слой PII; нет пакета → SKIP."""
    try:
        from presidio_analyzer import AnalyzerEngine  # type: ignore
    except Exception:
        return {"check": "pii_presidio", "status": "SKIP", "severity": "high",
                "detail": "presidio-analyzer не установлен", "evidence": {}}
    try:
        sample = " ".join(df.astype(str).head(200).agg(" ".join, axis=1).tolist())
        engine = AnalyzerEngine()
        hits = engine.analyze(text=sample, language="en")
        types = sorted({h.entity_type for h in hits})
        return {"check": "pii_presidio",
                "status": "FAIL" if types else "PASS",
                "severity": "high",
                "detail": f"Presidio: {types}" if types else "Presidio: PII не найдено",
                "evidence": {"entity_types": types, "count": len(hits)}}
    except Exception as e:  # noqa: BLE001
        return {"check": "pii_presidio", "status": "SKIP", "severity": "high",
                "detail": f"Presidio error: {e}", "evidence": {}}
...
def gate_check(path: str, schema: list[str] | None = None) -> list[dict]:
    ...
    # 8b. Presidio (опционально — усиление PII)
    results.append(_check_presidio_pii(df))
